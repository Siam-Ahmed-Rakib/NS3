...
NS_LOG_COMPONENT_DEFINE("PaperTopologySim");
...
main(int argc, char* argv[])
{
    ...
    cmd.AddValue("tcp", "TCP variant: ml, plus, or newreno", tcp);
    cmd.AddValue("nNodes", "Number of mobile nodes", nNodes);
    cmd.AddValue("nFlows", "Number of concurrent TCP flows", nFlows);
    cmd.AddValue("simTime", "Simulation time in seconds", simTime);
    cmd.AddValue("areaX", "Simulation area width (m)", areaX);
    cmd.AddValue("areaY", "Simulation area height (m)", areaY);
    ...
    cmd.AddValue("alpha", "Alpha smoothing (TcpWestwoodMl)", alpha);
    cmd.AddValue("ratioThreshold", "RTT ratio threshold (TcpWestwoodMl)", ratioThreshold);
    ...
    posAllocFactory.SetTypeId("ns3::RandomRectanglePositionAllocator");
    posAllocFactory.Set("X",
                        StringValue("ns3::UniformRandomVariable[Min=0.0|Max=" +
                                    std::to_string(areaX) + "]"));
    posAllocFactory.Set("Y",
                        StringValue("ns3::UniformRandomVariable[Min=0.0|Max=" +
                                    std::to_string(areaY) + "]"));
    Ptr<PositionAllocator> posAlloc = posAllocFactory.Create()->GetObject<PositionAllocator>();
    ...
    startRv->SetAttribute("Min", DoubleValue(10.0)); // leave time for AODV routes
    ...
    LogComponentEnable("PaperTopologySim", LOG_LEVEL_INFO);
