...
# Usage:  bash tools/run_experts_comparison.sh [short]


set -e
cd "$(dirname "$0")/.."

OUTDIR="results/experts_comparison"
mkdir -p "$OUTDIR"

TCPS=("experts" "ml" "plus" "newreno")
...
CSV="$OUTDIR/experts_comparison_results.csv"
...
for tcp in "${TCPS[@]}"; do
  for nf in "${FLOWS[@]}"; do
    for seed in "${SEEDS[@]}"; do
      ...
      # Parse the SUMMARY section only
...
echo ""
echo "Analyse with:  python3 tools/analyze_experts_comparison.py"
